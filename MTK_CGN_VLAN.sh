...
while [ $VLAN -le $endVLAN ]; do
    ...
    echo /interface vlan add interface=$interface name=$interface"_V"$VLAN vlan-id=$VLAN
    echo /interface bridge add name=BR_V$VLAN protocol-mode=none
    echo /interface bridge port add bridge=BR_V$VLAN interface=$interface"_V"$VLAN
